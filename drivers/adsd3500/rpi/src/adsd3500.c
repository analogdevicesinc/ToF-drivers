 ...
 *
 * Copyright (C) 2025 Analog Devices, All Rights Reserved.
 *
 */
...
struct adsd3500 {
	...
	struct v4l2_ctrl *load_firmware;
...
static const struct v4l2_ctrl_config adsd3500_firmware = {
	.ops    	= &adsd3500_ctrl_ops,
			...
			struct v4l2_subdev_format *format)
{
	...
	case V4L2_SUBDEV_FORMAT_TRY:
		return v4l2_subdev_state_get_format(state, format->pad);
			       ...
			       struct v4l2_subdev_format *format)
{
	...
	pad_format = adsd3500_get_pad_format(adsd3500, state, format);
...
static int adsd3500_init_ctrls(struct adsd3500 *priv)
{
	...
	priv->load_firmware = v4l2_ctrl_new_custom(&priv->ctrls,
						&adsd3500_firmware,
						NULL);
...
static int adsd3500_probe(struct i2c_client *client)
{
	...
	adsd3500->regmap = devm_regmap_init_i2c(client, &adsd3500_regmap_config);
	...
	ret= adsd3500_procfs_init(adsd3500);
	if(ret < 0){
